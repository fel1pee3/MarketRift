...
import { makeAnalysisJob } from '../src/analysis-job';

test('TypeScript analysis producer is accepted by Python consumer', () => {
  const job = makeAnalysisJob(
    'b522d3cb-556c-46f3-bca3-a4d9a3a75e69',
    'e8f28408-d57b-4839-989b-f519550c8e0d',
  );
  assert.deepEqual(Object.keys(job).sort(), ['document_id', 'extractor_version', 'idempotency_key', 'tenant_id', 'version']);
  const python = join(__dirname, '../../intelligence/.venv', process.platform === 'win32' ? 'Scripts/python.exe' : 'bin/python');
  const result = spawnSync(python, ['-m', 'marketrift_intelligence.contract_cli', 'analysis'], {
