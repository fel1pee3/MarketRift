...
test('GitHub source creation applies RBAC, tenant context and product visibility', async () => {
  for (const role of ['owner', 'admin', 'analyst', 'viewer'] as Role[]) {
    ...
    const db = { tenant: async (tenantId: string, callback: (client: object) => Promise<unknown>) => {
      ...
      return [{ id: source, product_id: product, source_type: 'github_issues', url: args[2] }];
    } } as unknown as Db;
    const controller = new ApiController(db, {} as Jobs, accounts);
    if (role === 'owner' || role === 'admin') {
      const created = await controller.createGitHubSource({} as Request,
        { product_id: product, repository: 'Example/Repo' });
      assert.equal(created.url, 'https://github.com/example/repo');
      assert.equal(seen[0], tenant);
      assert.match(seen[1]!, /FROM marketrift.products WHERE id = \$2/);
    } else {
      await assert.rejects(controller.createGitHubSource({} as Request,
...
test('analyst may sync; viewer cannot; invisible source is rejected', async () => {
  const accounts = { principal: async (_request: Request, allowed: Role[]) => {
  ...
  const controller = new ApiController(db, {} as Jobs, accounts);
  await assert.rejects(controller.syncGitHubSource({} as Request, source,
    { max_pages: 1, max_items: 1 }), { status: 404 });
  await assert.rejects(controller.syncGitHubSource({} as Request, source,
    { max_pages: 4, max_items: 1 }), { status: 400 });
  const viewer = { principal: async (_request: Request, allowed: Role[]) => {
    if (!allowed.includes('viewer')) throw Object.assign(new Error('Forbidden'), { status: 403 });
    return { tenantId: tenant, role: 'viewer' };
  } } as unknown as Accounts;
  const viewerController = new ApiController(db, {} as Jobs, viewer);
  await assert.rejects(viewerController.syncGitHubSource({} as Request, source,
    { max_pages: 1, max_items: 1 }), { status: 403 });
});
