...
De volta à raiz, `db:setup` está configurado para aplicar as doze migrações em um **banco vazio** e criar logins distintos de runtime e provisionamento. Não o execute em um banco existente:
...
São **doze migrações do mesmo banco PostgreSQL**, não bancos alternativos. `001_initial.sql` cria a base; `002_full_product.sql` acrescenta o domínio do produto completo; `003_first_slice.sql` acrescenta senha, fontes manuais e marcação sintética; `004_account_security.sql` acrescenta sessões revogáveis e convites; `005_review_analysis.sql` acrescenta análises versionadas e múltiplos problemas por avaliação; `006_github_issues.sql` acrescenta fonte, documentos e estado de coleta de Issues; `007_steam_reviews.sql` acrescenta fonte e metadados de reviews Steam; `008_web_pages.sql` acrescenta capturas de páginas; `009_page_monitoring.sql` acrescenta agendamento persistente e estado versionado da interpretação; `010_github_discussions.sql` acrescenta Discussions como tipo de fonte e documento independente; `011_b2b_review_rights.sql` acrescenta a separação B2B/G2, direitos e estado dos dados; `012_b2b_analysis_rights.sql` separa o direito de envio à IA e registra limites por operação. `db:setup` está configurado para 001 a 012 somente em banco novo; ele **não** foi executado nesta sessão. Os comandos `db:migrate:*` aplicam somente a migração respectiva em banco existente, na ordem.
...
**Modelo semântico local opcional, ainda não executado aqui:** [`paraphrase-multilingual-MiniLM-L12-v2`](https://huggingface.co/sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2/tree/e8f8c211226b894fcb81acc59f3b34ba3efd5f42), revisão fixa `e8f8c211...`, Apache-2.0, 384 dimensões, 50 idiomas e peso `model.safetensors` de **471 MB** (fora dependências). Instale o extra Python `embeddings`, execute explicitamente `npm run prepare:embeddings` uma vez, configure `EMBEDDING_PROVIDER=local` e `EMBEDDING_MODEL_PATH` para o diretório baixado, reinicie worker/HTTP e reindexe as fontes. Esses passos opcionais de instalação/download/inferência **não foram executados nesta entrega**; não trate o modo controlado como validação da qualidade semântica. Pesos em `.models/` são ignorados pelo Git. Consulte [ADR 0015](docs/adr/0015-busca-vetorial-e-respostas-extrativas.md) para direitos, limites e fontes técnicas.
